...
class KnowledgeMain extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<HomeBloc>().add(GetKnowledge(context: context));
    });
                    ...
                    return  SingleChildScrollView(
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          var item = state.knowledgeList[index];
                          ...
                          return GestureDetector(
                            onTap:(){
                              if(index==0){
                                context.pushNamed(Routes.discussion);
                              }
                              else if(index==1){
                                context.pushNamed(Routes.affirmation);
                              }
                            },
                            child: KnowledgeContainerView(
                              image: imageUrl!,
                              title: item.title!,
